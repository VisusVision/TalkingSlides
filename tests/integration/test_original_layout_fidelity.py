...
def test_custom_mode_without_custom_background_falls_back_to_whiteboard(tmp_path, monkeypatch):
    whiteboard_path = str(tmp_path / "custom-missing-fallback.png")
    ...
    assert calls["create"][0]["image_path"] != original_image_path
    assert result["scene_background_mode"] == "whiteboard"
    assert result["whiteboard_mode"] is True
    assert "custom_background_missing_fallback_whiteboard" in result["source_render_warnings"]
...
def test_non_pptx_source_background_falls_back_to_whiteboard(tmp_path, monkeypatch):
    whiteboard_path = str(tmp_path / "docx-source-background-fallback.png")
    calls = _patch_render_dependencies(monkeypatch, whiteboard_result=whiteboard_path)
    slide_meta = _base_slide_meta(tmp_path, mode="source_background", whiteboard_mode=False)
    slide_meta["source_type"] = "docx"
    original_image_path = slide_meta["image_path"]
    ...
    assert result["scene_background_mode"] == "whiteboard"
    assert result["whiteboard_mode"] is True
    assert "source_background_unsupported_for_source_type" in result["source_render_warnings"]
...
def test_docx_original_mode_does_not_overlay_text(tmp_path, monkeypatch):
    calls = _patch_render_dependencies(monkeypatch)
    slide_meta = _base_slide_meta(tmp_path, mode="original", whiteboard_mode=False)
    slide_meta["source_type"] = "docx"
    original_image_path = slide_meta["image_path"]
