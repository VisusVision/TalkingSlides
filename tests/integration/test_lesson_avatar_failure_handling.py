...
def patched_slide_dependencies(tmp_path, monkeypatch):
    ...
    def fake_compose_slide_with_avatar(slide_image_path, avatar_video_path, audio_path, output_path, **kwargs):
        ...
        Path(output_path).parent.mkdir(parents=True, exist_ok=True)
        Path(output_path).write_bytes(b"slide-video-with-avatar")
    ...
    monkeypatch.setattr(ffmpeg_helpers, "compose_slide_with_avatar", fake_compose_slide_with_avatar)
...
def test_slide_render_attaches_avatar_when_segment_succeeds(tmp_path, monkeypatch, patched_slide_dependencies):
    ...
    assert result["avatar_visible"] is True
    assert Path(result["part_path"]).read_bytes() == b"slide-video-with-avatar"
    assert patched_slide_dependencies["compose"][0]["avatar_video_path"] == str(avatar_output)
    assert patched_slide_dependencies["compose"][0]["avatar_layout"]["position"] == "top-right"


def test_slide_render_does_not_burn_hidden_avatar_into_final_part(tmp_path, monkeypatch, patched_slide_dependencies):
    slide_image = tmp_path / "slide.png"
    slide_image.write_bytes(b"png")
    source_image = tmp_path / "avatars" / "teacher.png"
    source_image.parent.mkdir(parents=True)
    ...
    result = worker_tasks.synthesize_and_render_slide.run(
        {
            ...
            "slide_num": 1,
            "image_path": str(slide_image),
            "notes_text": "A hidden avatar lesson slide.",
            "audio_out": str(tmp_path / "audio" / "slide_001.mp3"),
            "part_out": str(tmp_path / "parts" / "part_001.mp4"),
            "avatar_layout": {"visible": False},
        },
        project_id="129",
        voice_id="voice",
        pause_sec=0.0,
    ...
    assert result["avatar_visible"] is False
    assert Path(result["part_path"]).read_bytes() == b"slide-video"
    assert patched_slide_dependencies["compose"] == []
...
def test_concat_finalize_marks_burned_in_avatar_and_skips_duplicate_overlay(tmp_path, monkeypatch):
    ...
    def fake_concat_videos(_part_paths, output_path):
        Path(output_path).parent.mkdir(parents=True, exist_ok=True)
        Path(output_path).write_bytes(b"final-video-with-avatar")
    ...
    assert updates[-1]["status"] == "done"
    assert result["background_avatar"]["status"] == "baked"
    assert queued == []

    sidecar_payload = json.loads((tmp_path / "129" / "playback_assets.json").read_text(encoding="utf-8"))
    assert sidecar_payload["avatar_burned_in"] is True
    assert sidecar_payload["avatar"] is None
    assert sidecar_payload["final_segments"][0]["avatar_composited"] is True
