...
def test_admin_detail_includes_useful_finding_location_fields():
    ...
    assert response.data["open_project_studio_hint"] == f"/studio?lesson={project.id}"
    assert response.data["open_watch_timestamp_hint"] == f"/watch?lesson={project.id}&t=72.4"
