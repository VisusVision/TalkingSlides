...
- Renders slide videos, concatenates them into MP4 output, and generates SRT subtitles.
...
- `Studio.jsx`: teacher/publisher project upload, project list, Studio editor workspace, transcript/slides/notes/TTS panels, and rerender controls.
...
7. Worker concatenates output, writes SRT subtitles, syncs transcript timings, writes `playback_assets.json`, and marks the job done.
...
| Subtitle/SRT generation | Implemented | `services/scripts/ffmpeg_helpers.py`, `services/worker/tasks.py`, `tests/integration/test_pipeline_minimal.py` |
