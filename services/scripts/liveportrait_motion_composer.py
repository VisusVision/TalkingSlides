...
def _preset_settings(preset: str, *, motion_profile: str = "default") -> dict[str, Any]:
    ...
    if resolved == "subtle_blink":
        return {
            ...
            "blink_duration_s": float(_BLINK_DUR),
            "blink_shift_base_px": float(_BLINK_SHIFT_PX),
    ...
    return {
        ...
        "head_shift_cap_px": 0.95,
        "boosted_head_shift_cap_px": 1.05,
        "head_shift_min_px": 0.24,
        "blink_duration_s": max(float(_BLINK_DUR), 0.32),
        "blink_shift_base_px": max(float(_BLINK_SHIFT_PX), 0.48),
        "blink_shift_cap_px": 0.48 if not boosted_profile else 0.58,
        "blink_shift_min_px": 0.20,
...
def _build_motion_recipe(
    ...
    *,
    ...
    effective_blink_shift_px = _bounded(
        float(preset_settings.get("blink_shift_base_px", _BLINK_SHIFT_PX)) * float(scales["blink_shift"]),
    ...
    return {
        ...
        "blink_duration_s": round(float(preset_settings.get("blink_duration_s", _BLINK_DUR)), 4),
