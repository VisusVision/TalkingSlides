...
export default function Home({ searchQuery, user, onLoginRequest }) {
    ...
    async function loadDiscovery() {
      ...
      try {
        ...
        try {
          ...
          if (!active) return;
          setError(catalogError.message || t('dashboardFeedError'));
          setSections([]);
        }
      } finally {
        if (active) {
  ...
  const topicRailTitle = activeCategory === ALL_TOPICS
    ? t('exploreByTopic')
    : t('moreInCategory', { category: activeCategory });
  ...
  return (
    <div className="space-y-12 pb-10">
      {featuredLesson && (
        <section className="relative -mx-3 overflow-hidden rounded-[1.75rem] sm:-mx-6 lg:-mx-8" aria-label={t('dashboardFeaturedLesson')}>
          <LessonActionButton
      ...
      {loading && (
        <SurfaceCard elevated className="space-y-2">
          <p className="label-sm">{t('loadingEllipsis').replace('...', '')}</p>
          <p className="body-md">{t('dashboardLoading')}</p>
        </SurfaceCard>
      )}

      {error && (
        <SurfaceCard elevated className="space-y-2">
          <p className="label-sm">{t('feedNotice')}</p>
      ...
      {!loading && continueLessons.length > 0 && (
        <section className="space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="font-['Manrope'] text-2xl font-bold tracking-[-0.02em] text-[var(--text-primary)]">{t('historyTitle')}</h2>
            <button
              type="button"
      ...
      {!loading && filteredRecommended.length > 0 && (
        <section className="space-y-5 overflow-hidden">
          <div className="flex items-center justify-between">
            <h2 className="font-['Manrope'] text-2xl font-bold tracking-[-0.02em] text-[var(--text-primary)]">{t('recommendedForYou')}</h2>
            <div className="hidden items-center gap-2 sm:flex">
              <button
                type="button"
                aria-label={t('scrollRecommendedLeft')}
              ...
              <button
                type="button"
                aria-label={t('scrollRecommendedRight')}
                onClick={() => scrollRecommendedRail(1)}
            ...
            <article className="relative md:col-span-2 md:row-span-2">
              ...
              >
                ...
                <div className="relative flex h-full min-h-[320px] flex-col justify-end gap-2 p-6 pr-14">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[color:var(--media-text-on-image)]">{t('masterclass')}</p>
      ...
      {!loading && trendingLessons.length > 0 && (
        <section className="space-y-5">
          <h2 className="font-['Manrope'] text-2xl font-bold tracking-[-0.02em] text-[var(--text-primary)]">{t('trendingNow')}</h2>
      ...
      {!loading && allLessons.length === 0 && (
        <SurfaceCard elevated className="space-y-3 text-center">
          <p className="title-lg text-[var(--text-primary)]">{t('noLessonsMatched')}</p>
          <p className="body-md">{t('tryBroaderSearch')}</p>
        </SurfaceCard>
      )}
    </div>
  );
