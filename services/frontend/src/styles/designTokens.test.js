...
describe('frontend design tokens', () => {
  ...
  it('keeps shared motion utilities token-backed and reduced-motion safe', () => {
    ...
    expect(globalsCss).toContain('.motion-watch-complete');
    expect(globalsCss).toContain('.motion-watch-buffering-dot');
    expect(globalsCss).toContain('animation: motion-watch-pulse var(--duration-slow) var(--ease-in-out) infinite');
