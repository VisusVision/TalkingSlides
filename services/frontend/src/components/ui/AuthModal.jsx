  ...
  const handleGoogleRedirect = async () => {
    try {
      ...
      if (!data?.authorization_url) {
        throw new Error(t('authGoogleUnavailable'));
      }
      window.location.href = data.authorization_url;
    } catch (err) {
      setError(err.message || t('authGoogleFailed'));
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[color:var(--modal-backdrop)] p-4 backdrop-blur-sm">
      <SurfaceCard className="relative w-full max-w-md" elevated>
        <button
          ...
          aria-label={t('authCloseSignIn')}
        >
          <X size={16} />
        </button>

        <p className="label-sm">{t('authWelcome')}</p>
        <h2 className="headline-md mt-2 text-[var(--text-primary)]">{t('authTitle')}</h2>
        <p className="body-md mt-2">{t('authHelper')}</p>
        ...
        <form className="mt-5 space-y-3" onSubmit={handleSubmit}>
          ...
          <Button className="w-full" type="submit" disabled={loading}>
            <LogIn size={16} />
            <span>{loading ? t('authSigningIn') : t('authSubmit')}</span>
          </Button>
        </form>
      </SurfaceCard>
    </div>
