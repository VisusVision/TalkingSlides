...
CELERY_DIGITAL_TWIN_TRAIN_QUEUE = os.environ.get("CELERY_DIGITAL_TWIN_TRAIN_QUEUE", "avatar-train")
CELERY_DIGITAL_TWIN_RENDER_QUEUE = os.environ.get("CELERY_DIGITAL_TWIN_RENDER_QUEUE", "avatar-render")
CELERY_DIGITAL_TWIN_VERIFY_QUEUE = os.environ.get("CELERY_DIGITAL_TWIN_VERIFY_QUEUE", "avatar-verify")
...
).strip() or CELERY_RENDER_QUEUE
INTELLIGENCE_CELERY_QUEUE = (
    os.environ.get("INTELLIGENCE_CELERY_QUEUE")
    or os.environ.get("CELERY_INTELLIGENCE_QUEUE")
    or INTELLIGENCE_CELERY_QUEUE_DEFAULT
).strip() or INTELLIGENCE_CELERY_QUEUE_DEFAULT
CELERY_INTELLIGENCE_QUEUE = INTELLIGENCE_CELERY_QUEUE
INTELLIGENCE_LESSON_CELERY_QUEUE = (
    os.environ.get("INTELLIGENCE_LESSON_CELERY_QUEUE")
    or INTELLIGENCE_CELERY_QUEUE
).strip() or INTELLIGENCE_CELERY_QUEUE
INTELLIGENCE_ANALYTICS_CELERY_QUEUE = (
    os.environ.get("INTELLIGENCE_ANALYTICS_CELERY_QUEUE")
    or INTELLIGENCE_CELERY_QUEUE
).strip() or INTELLIGENCE_CELERY_QUEUE
INTELLIGENCE_RECOMMENDED_DEDICATED_QUEUE = (
    os.environ.get("INTELLIGENCE_RECOMMENDED_DEDICATED_QUEUE", "intelligence").strip()
    or "intelligence"
)
...
LESSON_PROTECTION_UNCONFIRMED_GRANT_GRACE_SECONDS = int(os.environ.get("LESSON_PROTECTION_UNCONFIRMED_GRANT_GRACE_SECONDS", "30"))
...
INTELLIGENCE_SYNC_PROVIDER_TIMEOUT_CAP_SECONDS = float(
    os.environ.get("INTELLIGENCE_SYNC_PROVIDER_TIMEOUT_CAP_SECONDS", "20")
)
_INTELLIGENCE_PROFILE_DEFAULTS = {
    ...
    "production_gpu": {
        "lesson_model": "qwen3:14b",
        "analytics_model": "qwen3:8b",
        "chunk_chars": "10000",
        "chunk_pages": "12",
        "chunk_items": "16",
        "row_threshold": "80",
...
INTELLIGENCE_HARDWARE_PROFILE = os.environ.get("INTELLIGENCE_HARDWARE_PROFILE", "local_mid").strip().lower()
if INTELLIGENCE_HARDWARE_PROFILE not in _INTELLIGENCE_PROFILE_DEFAULTS:
    INTELLIGENCE_HARDWARE_PROFILE = "local_mid"
_INTELLIGENCE_PROFILE = _INTELLIGENCE_PROFILE_DEFAULTS[INTELLIGENCE_HARDWARE_PROFILE]
INTELLIGENCE_BACKGROUND_ENHANCEMENT_ENABLED = os.environ.get(
    "INTELLIGENCE_BACKGROUND_ENHANCEMENT_ENABLED",
    "true",
).lower() in {"1", "true", "yes", "on"}
INTELLIGENCE_BACKGROUND_PROVIDER_TIMEOUT_SECONDS = float(
    os.environ.get("INTELLIGENCE_BACKGROUND_PROVIDER_TIMEOUT_SECONDS", "120")
)
INTELLIGENCE_BACKGROUND_TIMEOUT_MIN_SECONDS = float(
    os.environ.get("INTELLIGENCE_BACKGROUND_TIMEOUT_MIN_SECONDS", "60")
)
INTELLIGENCE_BACKGROUND_TIMEOUT_MAX_SECONDS = float(
    os.environ.get("INTELLIGENCE_BACKGROUND_TIMEOUT_MAX_SECONDS", "300")
)
INTELLIGENCE_BACKGROUND_TIMEOUT_PER_1000_CHARS = float(
    os.environ.get("INTELLIGENCE_BACKGROUND_TIMEOUT_PER_1000_CHARS", "4")
)
INTELLIGENCE_BACKGROUND_TIMEOUT_PER_PAGE_SECONDS = float(
    os.environ.get("INTELLIGENCE_BACKGROUND_TIMEOUT_PER_PAGE_SECONDS", "2")
)
INTELLIGENCE_BACKGROUND_TIMEOUT_PER_COMMENT_SECONDS = float(
...
INTELLIGENCE_OLLAMA_CHUNK_MAX_PAGES = int(
    os.environ.get("INTELLIGENCE_OLLAMA_CHUNK_MAX_PAGES", _INTELLIGENCE_PROFILE["chunk_pages"])
)
INTELLIGENCE_OLLAMA_CHUNK_MAX_ITEMS = int(
    os.environ.get("INTELLIGENCE_OLLAMA_CHUNK_MAX_ITEMS", _INTELLIGENCE_PROFILE["chunk_items"])
)
INTELLIGENCE_OLLAMA_CHUNK_ROW_THRESHOLD = int(
    os.environ.get("INTELLIGENCE_OLLAMA_CHUNK_ROW_THRESHOLD", _INTELLIGENCE_PROFILE["row_threshold"])
)
INTELLIGENCE_OLLAMA_CHUNK_CONCURRENCY = int(
    os.environ.get("INTELLIGENCE_OLLAMA_CHUNK_CONCURRENCY", _INTELLIGENCE_PROFILE["chunk_concurrency"])
)
INTELLIGENCE_OLLAMA_CHUNK_TIMEOUT_MIN_SECONDS = float(
    os.environ.get("INTELLIGENCE_OLLAMA_CHUNK_TIMEOUT_MIN_SECONDS", _INTELLIGENCE_PROFILE["chunk_timeout_min"])
)
INTELLIGENCE_OLLAMA_CHUNK_TIMEOUT_MAX_SECONDS = float(
    os.environ.get("INTELLIGENCE_OLLAMA_CHUNK_TIMEOUT_MAX_SECONDS", _INTELLIGENCE_PROFILE["chunk_timeout_max"])
)
...
LESSON_INTELLIGENCE_SYNC_PROVIDER_TIMEOUT_CAP_SECONDS = float(
    os.environ.get(
        "LESSON_INTELLIGENCE_SYNC_PROVIDER_TIMEOUT_CAP_SECONDS",
        str(INTELLIGENCE_SYNC_PROVIDER_TIMEOUT_CAP_SECONDS),
    )
)
LESSON_INTELLIGENCE_BACKGROUND_PROVIDER_TIMEOUT_SECONDS = float(
    os.environ.get(
        "LESSON_INTELLIGENCE_BACKGROUND_PROVIDER_TIMEOUT_SECONDS",
        str(INTELLIGENCE_BACKGROUND_PROVIDER_TIMEOUT_SECONDS),
    )
)
...
ANALYTICS_INTELLIGENCE_SYNC_PROVIDER_TIMEOUT_CAP_SECONDS = float(
    os.environ.get(
        "ANALYTICS_INTELLIGENCE_SYNC_PROVIDER_TIMEOUT_CAP_SECONDS",
        str(INTELLIGENCE_SYNC_PROVIDER_TIMEOUT_CAP_SECONDS),
    )
)
ANALYTICS_INTELLIGENCE_BACKGROUND_PROVIDER_TIMEOUT_SECONDS = float(
    os.environ.get(
        "ANALYTICS_INTELLIGENCE_BACKGROUND_PROVIDER_TIMEOUT_SECONDS",
        str(INTELLIGENCE_BACKGROUND_PROVIDER_TIMEOUT_SECONDS),
    )
)
...
ANALYTICS_INTELLIGENCE_AUTO_ENABLED = os.environ.get(
    "ANALYTICS_INTELLIGENCE_AUTO_ENABLED",
    "true",
).lower() in {"1", "true", "yes", "on"}
ANALYTICS_INTELLIGENCE_MIN_AUTO_INTERVAL_SECONDS = int(
    os.environ.get("ANALYTICS_INTELLIGENCE_MIN_AUTO_INTERVAL_SECONDS", "3600")
)
ANALYTICS_INTELLIGENCE_MIN_PROGRESS_EVENT_DELTA = int(
    os.environ.get("ANALYTICS_INTELLIGENCE_MIN_PROGRESS_EVENT_DELTA", "5")
)
ANALYTICS_INTELLIGENCE_RECENT_COMMENTS_LIMIT = int(
    os.environ.get("ANALYTICS_INTELLIGENCE_RECENT_COMMENTS_LIMIT", "20")
)
ANALYTICS_INTELLIGENCE_COMMENT_MAX_CHARS = int(
    os.environ.get("ANALYTICS_INTELLIGENCE_COMMENT_MAX_CHARS", "280")
)
