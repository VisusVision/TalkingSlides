...
Keep `STORAGE_BACKEND=filesystem` for normal coworker setup. Compose MinIO is available for optional S3 adapter readiness checks only; it does not serve runtime media by default.
