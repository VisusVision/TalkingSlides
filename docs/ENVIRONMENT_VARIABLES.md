...
| `AVATAR_MUSETALK_SERVICE_ENABLED`, `AVATAR_MUSETALK_SERVICE_PORT`, `AVATAR_MUSETALK_ROUTE` | worker-avatar | If avatar | If avatar | service route values | Persistent MuseTalk service behavior. Service logs are attached to PID 1 descriptors so a short-lived bootstrap caller cannot break the service output pipe. |
