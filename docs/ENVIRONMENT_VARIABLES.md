...
| `AVATAR_MUSETALK_SERVICE_ENABLED`, `AVATAR_MUSETALK_SERVICE_PORT`, `AVATAR_MUSETALK_ROUTE` | worker-avatar | If avatar | If avatar | service route values | Persistent MuseTalk service behavior. Service logs are attached to PID 1 descriptors so a short-lived bootstrap caller cannot break the service output pipe. Before GPU inference, the service process verifies its own write access to the requested output directory and rejects permission failures as non-retryable HTTP 422 responses. |
| `AVATAR_MUSETALK_TRANSIENT_RETRY_COUNT`, `AVATAR_MUSETALK_TRANSIENT_RETRY_DELAY_SECONDS` | worker-avatar | Optional | Recommended | `1`, `1` | Retries only transient service transport failures, at most twice. The existing LivePortrait handoff is reused; timeout, CUDA/OOM, model, and output-validation failures are not retried. |
| `MUSETALK_PREFLIGHT_MIN_OUTPUT_FREE_MIB`, `MUSETALK_PREFLIGHT_MIN_TEMP_FREE_MIB`, `MUSETALK_PREFLIGHT_TEMP_MIB_PER_FRAME` | worker-avatar | Optional | Recommended | `256`, `1024`, `4` | Fail-fast storage capacity thresholds. Temporary demand is the larger of its floor or the estimated frame count multiplied by MiB per frame. Insufficient capacity returns non-retryable HTTP 507 before the GPU lock is acquired. |
| `MUSETALK_IDEMPOTENCY_TTL_SECONDS`, `MUSETALK_IDEMPOTENCY_MAX_ENTRIES`, `MUSETALK_IDEMPOTENCY_WAIT_TIMEOUT_SECONDS` | worker-avatar | Optional | Recommended | `3600`, `128`, `0` | Bounds same-run MuseTalk deduplication. Matching in-flight requests join the owner; completed results remain replayable for the TTL while the output exists. `0` derives the join timeout from the request HTTP budget. Same `run_id` with a different fingerprint returns non-retryable HTTP 409. |
